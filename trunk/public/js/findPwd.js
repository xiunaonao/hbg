(function(){
	...
	_app.click('#postcodeBtn',function(t,e){
		...
		_app.AJAX('/api/getCode','POST',{mobile:username.value,type:'finwPwd'},function(data){
	...
	_app.click('#resetBtn',function(t,e){
		...
		_app.AJAX('/api/findPwd','POST',form,function(data){
			var data=JSON.parse(data);
			if(data.code==0){
				if(confirm('重置密码成功，是否去登录？')){	
					location.href='login';
				}
			}else{
				alert(data.msg);
